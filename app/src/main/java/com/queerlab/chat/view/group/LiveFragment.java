...
//            groupViewModel.voiceRoomDelete(listBean.getGroup_no());
        });

        groupViewModel = mActivity.getViewModel(GroupViewModel.class);

        //获取live小组成功返回结果
        groupViewModel.groupLiveListLiveData.observe(mActivity, groupListBean -> {
            isInited = true;
        ...
        smartRefreshLayout.setOnRefreshLoadMoreListener(new OnRefreshLoadMoreListener() {
            ...
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                groupViewModel.groupLiveList();
            }

            @Override
            public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
                groupViewModel.groupLiveListMore();
            }
        });
